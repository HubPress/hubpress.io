...
- Generalizing this, the value of \(X. Y\) will increase as long as we go on the direction of line that joins \((0, 0)\) to midpoint of the line \(X \times G + Y \times W = M\). The mid point of this line is, unsurprisingly, \(M/2G, M/2W)\)
