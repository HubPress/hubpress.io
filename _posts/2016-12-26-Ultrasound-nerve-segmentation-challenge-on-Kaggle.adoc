...
The training set is small as is, just 5635 samples. After discards, we are left with . This certainly doesn't look good for training a deep model, which is very data hungry. On top of that, we cannot use transfer learning either, as there are no nets trained on this sort of data set.
